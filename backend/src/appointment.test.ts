...
describe('IDOR-01 Security Regression Tests: Facility Queue (getFacilityQueue)', () => {
  ...
  beforeAll(async () => {
    // 1. Create Users
    await request(app).post('/api/auth/__test_provision').send({ username: 'fadminA_test', password: 'StrongP@ssw0rd!', role: 'ROLE_FACILITY_ADMIN' });
    const userA = db.prepare("SELECT id FROM users WHERE username = 'fadminA_test'").get().id;

    await request(app).post('/api/auth/__test_provision').send({ username: 'fadminB_test', password: 'StrongP@ssw0rd!', role: 'ROLE_FACILITY_ADMIN' });
    const userB = db.prepare("SELECT id FROM users WHERE username = 'fadminB_test'").get().id;

    await request(app).post('/api/auth/__test_provision').send({ username: 'dadmin_test', password: 'StrongP@ssw0rd!', role: 'ROLE_DISTRICT_ADMIN' });
